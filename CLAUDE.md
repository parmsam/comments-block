...
- "Allow unsigned extensions" lives in **Safari → Settings → Developer** (a settings tab), not the classic Develop menu bar item.
- After each Safari restart, two things reset (unsigned extension limitation): (1) the extension disappears from Safari's Extensions list — re-run the wrapper app or `reregister-safari-extension.sh`/`.command` to re-register it; (2) "Allow unsigned extensions" must be re-enabled from Safari → Settings → Developer. **Enable it before running the re-register script/command**, not after — the extension won't register correctly if it's off at run time. This toggle cannot be automated: checking it triggers a macOS sign-in/auth prompt (a hard security wall). Two automation attempts were tried and abandoned for this reason — a persistent LaunchAgent, and an AppleScript/System Events-driven click. Permanent fix: sign with an Apple Developer account ($99/year).
